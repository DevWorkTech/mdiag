...
Переработка входа **7.00.014-mdiag2**: [обновление и диагностика](REWORK_TRANSPORT_RU.md).
После обновления существующей установки выполните `php artisan migrate --force`: добавлена локальная информация профиля/мастерской.
...
RepairData/FAQ сохраняют HTML и связанные CSS/JS/изображения с локальными ссылками.
Динамический поиск, формы и серверные API требуют отдельных адаптеров. Customers/Workshop — явные заготовки с 501, без фиктивных данных/успеха.
...
Источники только xdiagpro.com/www/repairdata; до 5 редиректов внутри того же hostname
проверяются вручную перед каждым запросом. Парковочная страница отклоняется.
...
Снимки хранятся в storage/app/private/mdiag/web/<module>/; current.json — текущая
карта URL → SHA-256 файла. Старые current.txt требуют повторной синхронизации.
Старые версии сохраняются. Маршруты читают только локальные файлы, HTTP proxy отсутствует.
Лимиты и ограничения зеркала описаны в server/WEB_CONTENT_RU.md.
Штатные проверки сети восстановлены; подробности входа/TLS — android/NETWORK_RU.md.
